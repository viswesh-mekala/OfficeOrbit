...
export const Attendance: React.FC = () => {
  ...
  const selectedLiveMinutes = useMemo(
    () => liveDurationMinutes(selectedLog),
    [selectedLog],
  );
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.legendContainer}>
          <Text style={styles.legendTitle}>Color coding</Text>
            ...
            {['present', 'wfh', 'leave', 'holiday', 'weekend'].map((key) => (
              <View key={key} style={styles.legendItem}>
                <View
                  style={[
                    styles.legendSwatch,
                    { backgroundColor: STATUS_THEME[key].fill },
                  ]}
                />
            ...
            <View>
              ...
              <View style={styles.statusRow}>
                <View
                  style={[
                    styles.statusBadge,
                    { backgroundColor: selectedStatusTheme.fill },
                  ]}
                >
                  <Text
                    style={[
                      styles.statusBadgeText,
                      { color: selectedStatusTheme.text },
                    ]}
                  >
                    {getStatusLabel(selectedLog)}
                  </Text>
                </View>
                {typeof selectedLiveMinutes === 'number' ? (
                  <Text style={styles.statusTime}>
                    • {Math.floor(selectedLiveMinutes / 60)}h{' '}
                    {selectedLiveMinutes % 60}m
                    {!selectedLog?.check_out ? ' (live)' : ''}
                  </Text>
                ) : selectedLog?.duration_minutes ? (
                  <Text style={styles.statusTime}>
                    • {Math.floor(selectedLog.duration_minutes / 60)}h{' '}
                    {selectedLog.duration_minutes % 60}m
                  </Text>
                ) : null}
              </View>
              {isCalendarManagedDay(selectedLog) ? (
                <Text style={styles.calendarHint}>
                  Status changed successfully for the current date.
                </Text>
              ) : null}
            </View>
            <TouchableOpacity
              style={styles.editButton}
              onPress={handleEditSelectedDay}
            ...
            <View style={styles.modalOptions}>
              ...
              >
                <View
                  style={[
                    styles.optionDot,
                    { backgroundColor: STATUS_THEME.present.fill },
                  ]}
                />
              ...
              <TouchableOpacity
                style={styles.modalOption}
                onPress={() => handleStatusUpdate('wfh')}
              >
                <View
                  style={[
                    styles.optionDot,
              ...
              >
                <View
                  style={[
                    styles.optionDot,
                    { backgroundColor: STATUS_THEME.holiday.fill },
                  ]}
                />
              ...
              >
                <View
                  style={[
                    styles.optionDot,
                    { backgroundColor: STATUS_THEME.leave.fill },
                  ]}
                />
